...
function getGroupKey(expense: Expense, mode: ExpensePart): string {
  switch (mode) {
    ...
    case "person":
      return expense.assignedMemberId &&
        !isSplitAssignment(expense.assignedMemberId)
...
function sortExpenseList(
  ...
  if (sortByProp === "date") {
    ...
    updated.sort(
      (a, b) =>
        direction *
        getGroupKey(a, sortByProp).localeCompare(getGroupKey(b, sortByProp)),
    );
  }
  return updated;
}
...
function expensePayload(expense: Expense) {
  return {
    ...
    notes: expense.notes || "",
    assignedMemberId: isSplitAssignment(expense.assignedMemberId)
      ? null
      : expense.assignedMemberId,
  };
}
...
function matchesSearch(
  ...
  if (filterBy === "date")
    return formatDate(expense.date).toLowerCase().includes(term);
  if (filterBy === "account")
    return (expense.account || "").toLowerCase().includes(term);
  if (filterBy === "vendor")
    return (expense.vendor || "").toLowerCase().includes(term);
  if (filterBy === "category")
    return (expense.category || "").toLowerCase().includes(term);
  if (filterBy === "person")
    return (personName?.(expense) || "split").toLowerCase().includes(term);
  if (filterBy === "amount")
    return expense.amount.toString().includes(searchTerm);
  if (filterBy === "notes")
    return (expense.notes || "").toLowerCase().includes(term);
  return true;
}
...
function filteredExpenseList(
  ...
  if (searchTerm) {
    list = list.filter((expense) =>
      matchesSearch(expense, filterBy, searchTerm, personName),
    );
  }
  if (grouping !== "none") {
...
const Expenses = () => {
  ...
  const [editAddingCategoryId, setEditAddingCategoryId] = useState<
    string | null
  >(null);
  ...
  const [importPendingCategories, setImportPendingCategories] = useState<
    string[]
  >([]);
  const [importSuggested, setImportSuggested] = useState<boolean[]>([]);
  const [importCorrectingIndex, setImportCorrectingIndex] = useState<
    number | null
  >(null);
  const [importPage, setImportPage] = useState(1);
  const [addingCategoryIndex, setAddingCategoryIndex] = useState<number | null>(
    null,
  );
      ...
      .then((response) => {
        ...
        setVendorRules((current) => [
          ...
          ...current.filter(
            (rule) =>
              !(
                rule.matchType === saved.matchType &&
                rule.matchKey === saved.matchKey
              ),
          ),
        ]);
      })
  ...
  useEffect(() => {
    ...
    // Reload the visible table when the active profile changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeProfileId]);

  useEffect(() => {
    showExpenses(expensesCopy);
    // Keep the current page/filter when cached expenses update.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [expensesCopy, expensesLoading]);
  ...
  function showExpenses(
    ...
    const list = filteredExpenseList(
      all,
      range,
      filterBy,
      query,
  ...
  function AddExpense(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      const data = {
        ...
        notes,
        assignedMemberId:
          assignedMemberId === SPLIT_VALUE ? null : assignedMemberId,
      };
      api
        ...
        .then((res) => {
          ...
          const created = mapExpense({
            _id: res.data._id,
            ...data,
            assignedMemberId:
              res.data.assignedMemberId ?? data.assignedMemberId,
          });
  ...
  async function EditExpenses() {
    ...
    const changed = rows.filter((item) =>
      expenseChanged(item, originals.get(item._id)),
    );
    if (changed.length === 0) {
      setEditMode(false);
      setEditId(null);
    ...
    try {
      ...
      const byId = new Map(
        changed.map((item) => [
          ...
          {
            ...item,
            assignedMemberId: isSplitAssignment(item.assignedMemberId)
              ? null
  ...
  function loadCsvText(text: string) {
    ...
    const accounts = [
      ...new Set(
        expensesCopy.map((expense) => expense.account).filter(Boolean),
      ),
    ];
    setImportSource("csv");
  ...
  function loadPdfResult(data: StatementParseResponse) {
    const drafts = (data.expenses || [])
      .map((row) => ({
        date:
          parseDate(String(row.date)) ||
          new Date(`${String(row.date).slice(0, 10)}T00:00:00.000Z`),
  ...
  function continueToCategories() {
    ...
    if (csvResult.expenses.length === 0) {
      setImportError(
        "No valid expenses to import with the current column mapping.",
      );
      return;
    }
    ...
    const existingCategories = expensesCopy
      .map((expense) => expense.category)
      .filter(Boolean);
    const drafts = csvResult.expenses
      .map((expense) => ({
        ...expense,
        account: expense.account || importAccount.trim(),
        category:
          reuseExistingCategory(expense.category, existingCategories) ||
          "Uncategorized",
      }))
  ...
  function continueFromPreview() {
    ...
    if (
      !importHasAccount &&
      !importAccount.trim() &&
      importDrafts.some((expense) => !expense.account.trim())
    ) {
      setImportError("Please enter an account name for these transactions.");
      return;
    }
  ...
  function openCategoryStep(drafts: ParsedExpense[]) {
    ...
    const { drafts: predicted, suggested } = suggestDraftCategories(
      aligned,
      expensesCopy,
      vendorRules,
    );
    const fromFile = predicted
      .map((expense) => expense.category)
      .filter(Boolean);
    const fromExisting = existingCategories;
    const saved = uniqueCategoryNames(["Uncategorized", ...fromExisting]).sort(
      (a, b) => a.localeCompare(b),
    );
    const pending = uniqueCategoryNames(fromFile).filter(
      (name) =>
    ...
    setImportSelected((current) =>
      current.length === predicted.length ? current : predicted.map(() => true),
    );
  ...
  function setDraftCategory(
    ...
    setImportDrafts((current) => {
      ...
      setImportSuggested((currentSuggested) => {
        const next =
          currentSuggested.length === drafts.length
            ? [...currentSuggested]
            : drafts.map(() => false);
        next[index] = false;
  ...
  function confirmNewImportCategory(index: number) {
    const name = reuseExistingCategory(newImportCategory, [
      ...importCategories,
      ...importPendingCategories,
    ]);
    if (!name) return;
    const alreadySaved = importCategories.some(
    ...
    setDraftCategory(
      index,
      alreadySaved
        ? importCategories.find(
            (item) => item.toLowerCase() === name.toLowerCase(),
          ) || name
        : name,
      importCorrectingIndex === index,
    );
    setAddingCategoryIndex(null);
    setNewImportCategory("");
  }
  ...
  function negateAmountColumn() {
    ...
    setCsvPreview({
      ...
      rows: csvPreview.rows.map((row) => {
        ...
        next[amountIdx] = Number.isInteger(negated)
          ? String(negated)
          : String(Number(negated.toFixed(4)));
        return next;
      }),
    });
    setImportError("");
  ...
  function ImportExpenses() {
    const selectedDrafts = importDrafts.filter(
      (_, index) => importSelected[index],
    );
    ...
    if (selectedDrafts.some((expense) => !expense.category.trim())) {
      setImportError(
        "Please choose a category for every selected transaction.",
      );
      return;
    }
    ...
    setImportError("");
    api
      .post(`/api/expenses/import`, { expenses: expensesToImport })
      .then((res) => {
        closeImport();
        const created = (res.data.expenses || []).map(mapExpense);
        if (created.length > 0) {
          setExpensesCopy((current) => [...current, ...created]);
        } else {
          void refreshWorkspace();
        }
        showSuccess(
          `Imported ${res.data.count} expense${res.data.count === 1 ? "" : "s"}.`,
        );
      })
      .catch((err) => {
        reportError(err);
  ...
  async function DeleteExpenses() {
    ...
    const ok = await confirm({
      title:
        checkedExps.length === 1
          ? "Delete this expense?"
          : `Delete ${checkedExps.length} expenses?`,
    ...
    checkedExps.forEach((expense) => {
      api
        ...
        .then((res) => {
          const updatedExpensesCopy: Expense[] = expensesCopy.filter(
            (exp: Expense) => exp._id != res.data._id,
          );
          setExpensesCopy(updatedExpensesCopy);
          showExpenses(updatedExpensesCopy);
        })
  ...
  function handleAccount(e: React.ChangeEvent<HTMLSelectElement>) {
    ...
    if (e.target.selectedIndex == e.target.childElementCount - 1) {
      ...
      if (
        ...
        //if not a new account and not first option
  ...
  function confirmNewEditCategory(expense: Expense) {
    const name = reuseExistingCategory(editNewCategoryName, [
      ...existingCategories,
      ...editPendingCategories,
    ]);
    if (!name) return;
    const alreadySaved = existingCategories.some(
      (item) => item.toLowerCase() === name.toLowerCase(),
    );
    if (!alreadySaved) {
  ...
  function editDate(expense: Expense, d: string) {
    ...
    let updatedExpenses: Expense[] = editableExpenses.map((exp) => {
  ...
  function editAssigned(expense: Expense, memberId: string | null) {
    const updatedExpenses: Expense[] = editableExpenses.map((exp) => {
      if (exp._id === expense._id) {
  ...
  function filterBySearch(
    filterBy: string,
    searchTerm: string,
    grouping: ExpensePart = groupMode,
  ) {
  ...
  function renderExpenseRow(expense: Expense, bg: boolean) {
    if (editMode) {
      return (
        <ExpenseEditableRow
          ...
          onStartAddCategory={() => {
            setEditAddingCategoryId(expense._id);
            setEditNewCategoryName("");
          }}
    ...
    if (editId === expense._id) {
      const exp =
        editableExpenses.find((e) => e._id === expense._id) ?? expense;
      return (
        <ExpenseEditableRow
          ...
          selectClassName={cn(selectClassName, "h-8")}
          onStartAddCategory={() => {
            setEditAddingCategoryId(exp._id);
            setEditNewCategoryName("");
          }}
          onNewCategoryNameChange={setEditNewCategoryName}
  ...
  function renderTableBody() {
    ...
    return groups.map((group, index) => {
      ...
      const bg = index % 2 === 0;
      const countLabel =
        group.items.length === 1 ? "1 item" : `${group.items.length} items`;
      const groupIds = group.items.map((expense) => expense._id.toString());
      const selectedCount = groupIds.filter((id) =>
        checkedExps.includes(id),
      ).length;
      const allSelected =
      ...
      return (
        <React.Fragment key={`group-${group.key}-${index}`}>
          ...
          >
            {canWrite ? (
              <TableCell
                className={`${bg ? "bg-muted/40" : ""} w-10 text-center align-middle leading-none`}
                onClick={(event) => event.stopPropagation()}
              >
                <Checkbox
                  checked={someSelected ? "indeterminate" : allSelected}
                  onCheckedChange={(checked) =>
                    handleCheckGroup(groupIds, checked === true)
                  }
            ...
            <TableCell
              className={`${bg ? "bg-muted/40" : ""} font-semibold`}
              colSpan={7}
            >
              {groupLabel}{" "}
          ...
          {isOpen
            ? group.items.map((expense) => renderExpenseRow(expense, bg))
            : null}
        </React.Fragment>
      );
    });
  ...
  const importAllSelected =
    importSelected.length > 0 && importSelectedCount === importSelected.length;
    ...
    importStep === "categories"
      ...
      : importStep === "preview"
        ...
        : csvPreview
          ? "Import CSV"
          : "Import";
  const showImportUpload =
    importStep === "map" && !csvPreview && !importParsing;
      ...
      <main className="space-y-6 px-8 py-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {canWrite ? (
              <>
                ...
                {editMode || editId != null ? (
                  <>
                    <Button
                      ...
                      disabled={editAddingCategoryId !== null}
                    >
                      Save
                    </Button>
                    <Button
                ...
                <Button variant="destructive" onClick={DeleteExpenses}>
                  Delete
                </Button>
              </>
            ) : (
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Select
              value={groupMode}
              onValueChange={(value) => handleGroupBy(value as ExpensePart)}
            >
          ...
          <div className="flex min-w-[280px] items-center gap-2">
            <div className="relative flex-1">
              ...
              <Input
                ...
                aria-label="Search"
                onChange={(e) => {
                  setSearchByQuery(e.target.value);
                  filterBySearch(searchBySelect, e.target.value);
                }}
              />
            </div>
            <Select
          ...
          {expensesLoading ? (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  {canWrite ? (
                    <TableHead className="w-10 text-center align-middle leading-none">
                      <Checkbox
                        checked={
                          expenses.length > 0 &&
                          checkedExps.length === expenses.length
                        }
                        onCheckedChange={(checked) =>
                  ...
                  <TableHead>
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSortDate(sortDate === "down" ? "up" : "down");
                      }}
                    >
                      {sortDate === "down" ? (
                  ...
                  <TableHead className="w-[8.5rem] max-w-[8.5rem]">
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        sortBy("account", sortAccount);
                        setSortAccount(sortAccount === "down" ? "up" : "down");
                      }}
                    >
                      {sortAccount === "down" ? (
                  ...
                  <TableHead>
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        sortBy("vendor", sortVendor);
                        setSortVendor(sortVendor === "down" ? "up" : "down");
                      }}
                    >
                      {sortVendor === "down" ? (
                        <ArrowUpAZ className="h-4 w-4" />
                  ...
                  <TableHead>
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        sortBy("amount", sortAmount);
                        setSortAmount(sortAmount === "down" ? "up" : "down");
                      }}
                    >
                      {sortAmount === "down" ? (
                  ...
                  <TableHead className="w-[14rem] max-w-[14rem] pr-5">
                    <button
                      className="inline-flex items-center gap-1"
                      type="button"
                      onClick={() => {
                        sortBy("category", sortCategory);
                        setSortCategory(
                          sortCategory === "down" ? "up" : "down",
                        );
                      }}
                    >
                      {sortCategory === "down" ? (
                  ...
                  <TableHead>
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        sortBy("notes", sortNotes);
                        setSortNotes(sortNotes === "down" ? "up" : "down");
                      }}
                    >
                      {sortNotes === "down" ? (
                        <ArrowUpAZ className="h-4 w-4" />
            ...
            <div className="grid gap-3 py-4">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  {[
                    ...new Set(
                      expenses?.map((expense: Expense) => expense.account),
                    ),
                  ].map((account: string) => (
                    <option key={account} value={accCounter++}>
                      {account}
                    </option>
                  ))}
              ...
              <div className="space-y-2">
                ...
                <AttributionSelect
                  ...
                  selfMemberId={activeProfile?.memberId}
                  onChange={(memberId) =>
                    setAssignedMemberId(memberId || SPLIT_VALUE)
                  }
                />
              </div>
              <div className="space-y-2">
      ...
      >
        <DialogContent
          className={cn(
            ...
            importDialogWide &&
              "max-h-[90vh] w-[min(96vw,80rem)] sm:max-w-[80rem]",
            importDialogPreview &&
              "max-h-[90vh] w-[min(96vw,56rem)] sm:max-w-4xl",
            importDialogCategories &&
              "max-h-[90vh] w-[min(96vw,56rem)] sm:max-w-4xl",
          )}
        >
          <DialogHeader>
          ...
          >
              ...
              <>
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm text-muted-foreground">
                    Choose what each column maps to. Scroll to review the file
                    like a spreadsheet.
                  </p>
                ...
                />
                {csvMappingError && (
                  <p className="text-sm text-destructive">{csvMappingError}</p>
                )}
                {csvResult && (
                  <p className="text-sm text-muted-foreground">
                    {csvResult.expenses.length} expense
                    {csvResult.expenses.length === 1 ? "" : "s"} ready to import
                    {csvResult.skipped > 0
                      ? ` (${csvResult.skipped} row${csvResult.skipped === 1 ? "" : "s"} skipped)`
                      : ""}
                    .
                    {csvResult.hasCategory
                      ? " Categories from the mapped column will be applied and can be edited next. New names are created when you import."
                      : ""}
                  </p>
                )}
                {csvResult && !csvResult.hasAccount && (
                  <div className="space-y-2">
                    <Label htmlFor="importAccount">
                      Account for imported rows
                    </Label>
                    <Input
                      required
                      id="importAccount"
            ...
            {importStep === "preview" && (
              <>
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm text-muted-foreground">
                    Review extracted transactions. Uncheck rows you do not want
                    to import.
                  </p>
                  <Button
                    type="button"
                    ...
                    <Table className="table-fixed">
                        ...
                        <TableRow>
                          <TableHead className="w-10 text-center align-middle leading-none">
                            <Checkbox
                              ...
                              onCheckedChange={(checked) =>
                                toggleImportSelectedAll(checked === true)
                              }
                      ...
                      <TableBody>
                        {importDrafts
                          .slice(
                            importCategoryStart,
                            importCategoryStart + IMPORT_CATEGORY_PAGE_SIZE,
                          )
                          .map((expense, index) => {
                            const draftIndex = importCategoryStart + index;
                            const included =
                              importSelected[draftIndex] !== false;
                            return (
                              <TableRow
                                key={`import-preview-${draftIndex}`}
                                className={included ? "" : "opacity-50"}
                              >
                                <TableCell className="w-10 text-center align-middle leading-none">
                                  <Checkbox
                                    checked={included}
                                    onCheckedChange={(checked) =>
                ...
                {!importHasAccount && (
                  <div className="space-y-2">
                    <Label htmlFor="importPdfAccount">
                      Account for imported rows
                    </Label>
                    <Input
                      required
                              ...
                              >
                                ...
                                <TableCell className="w-[18rem]">
                                  <ImportCategorySelect
                                    ...
                                    isAdding={
                                      addingCategoryIndex === draftIndex
                                    }
                                    newName={newImportCategory}
                                    selectClassName={cn(selectClassName, "h-8")}
                                    suggestion={
                                      importSuggested[draftIndex]
                                        ? vendorFingerprint(expense.vendor)
                                        : ""
                                    }
                                    onChange={(category) =>
                                      setDraftCategory(
                                        draftIndex,
                                        category,
                                        importCorrectingIndex === draftIndex,
                                      )
                                    }
                                    onThumbsDown={() =>
                                      setImportCorrectingIndex(draftIndex)
                                    }
                                    onStartAdd={() => {
                                      setAddingCategoryIndex(draftIndex);
                                      setNewImportCategory("");
                                    }}
                                    onNewNameChange={setNewImportCategory}
                                    onConfirmAdd={() =>
                                      confirmNewImportCategory(draftIndex)
                                    }
                                    onCancelAdd={() => {
                                      setAddingCategoryIndex(null);
                                      setNewImportCategory("");
            ...
            {importStep === "categories" ? (
              <>
                <Button
                  ...
                  disabled={importing}
                  onClick={() => {
                    setImportStep(importSource === "pdf" ? "preview" : "map");
                    setImportError("");
                  }}
                >
                  Back
                ...
                >
                  {importing
                    ? "Importing..."
                    : `Import${importSelectedCount ? ` (${importSelectedCount})` : ""}`}
                </Button>
              </>
            ) : importStep === "preview" ? (
              <>
              ...
              <>
                ...
                {csvPreview && (
                  <Button
                    disabled={
                      importing ||
                      Boolean(csvMappingError) ||
                      !csvResult?.expenses.length
                    }
                    type="button"
                    onClick={continueToCategories}
                  >
...
function CsvSpreadsheetMapper({
  headers,
  rows,
  mapping,
  onChange,
}: CsvSpreadsheetMapperProps) {
  ...
  const cellClass =
    "border-r border-b border-border px-2 py-1.5 text-left text-xs";
            ...
            <tr>
              ...
              {hiddenColumns > 0 && (
                <th
                  className={cn(
                    cellClass,
                    "sticky right-0 z-20 min-w-[4.5rem] bg-muted p-1",
                  )}
                >
                  <button
                    type="button"
                    ...
                    onClick={() =>
                      setVisibleColumns((count) => count + CSV_PAGE_SIZE)
                    }
            ...
            <tr>
              ...
              {hiddenColumns > 0 && (
                <th
                  className={cn(
                    cellClass,
                    "sticky right-0 bg-muted text-center text-[11px] font-medium text-muted-foreground",
                  )}
                >
                  +{hiddenColumns}
                </th>
              ...
              <tr key={`row-${rowIndex}`} className="hover:bg-muted/40">
                ...
                {hiddenColumns > 0 && (
                  <td
                    className={cn(
                      cellClass,
                      "sticky right-0 bg-muted text-center text-muted-foreground",
                    )}
                  >
                    …
                  </td>
              ...
              <tr>
                ...
                >
                  <button
                    ...
                    onClick={() =>
                      setVisibleRows((count) => count + CSV_PAGE_SIZE)
                    }
                  >
                    <ChevronDown className="h-4 w-4" />
                    Show {Math.min(hiddenRows, CSV_PAGE_SIZE)} more rows
                    {hiddenRows > CSV_PAGE_SIZE
                      ? ` (${hiddenRows} remaining)`
                      : ""}
                  </button>
                </td>
              </tr>
...
function ImportCategorySelect({
  ...
  const options = uniqueCategoryNames([
    ...
    !pendingCategories.includes(value)
      ? [value]
      : []),
  ]);
  ...
  return (
    <div
      className={cn("flex h-8 w-full min-w-0 items-center gap-1", className)}
    >
      {isAdding ? (
        <Input
...
function ImportPageSpacer({
  ...
  return (
    <div aria-hidden style={{ height: `calc(${empty} * ${rowHeight})` }} />
  );
}
    ...
    <TableRow key={expense._id.toString()} className={bgClr}>
      <TableCell className="w-10 text-center align-middle leading-none">
        <Checkbox
          ...
          onCheckedChange={(checked) =>
            handleCheck(expense._id.toString(), checked === true)
          }
        />
      </TableCell>
      <TableCell>
        <Input
          ...
          onChange={(e) => {
            editDate(expense, e.target.value);
          }}
        />
      </TableCell>
      <TableCell className="w-[8.5rem] max-w-[8.5rem]">
        <Input
          ...
          onChange={(e) => {
            editAccount(expense, e.target.value);
          }}
        />
      </TableCell>
      <TableCell>
        <Input
          ...
          onChange={(e) => {
            editVendor(expense, e.target.value);
          }}
        />
      </TableCell>
      <TableCell>
        <Input
          ...
          onChange={(e) => {
            editAmount(expense, Number(e.target.value));
          }}
        />
      </TableCell>
      ...
      <TableCell className="pl-3">
        <AttributionSelect
          className={cn(
            selectClassName,
            "h-8 w-full min-w-0 max-w-full field-sizing-fixed",
          )}
      ...
      <TableCell>
        <Input
          className="h-8"
          type="text"
          value={expense.notes ? expense.notes : ""}
          onChange={(e) => {
            editNotes(expense, e.target.value);
          }}
        />
      </TableCell>
    </TableRow>
...
function ExpenseRow({
  ...
  return (
    ...
    >
      ...
      <TableCell className="w-[8.5rem] max-w-[8.5rem]">
        <span className="block truncate" title={expense.account}>
          {expense.account}
        </span>
      </TableCell>
      <TableCell>{expense.vendor}</TableCell>
      <TableCell>{expense.amount}</TableCell>
      <TableCell className="w-[14rem] max-w-[14rem] pr-5">
        <span className="block max-w-full truncate" title={expense.category}>
          {expense.category}
        </span>
      </TableCell>
      <TableCell className="pl-3">{assignedLabel}</TableCell>
