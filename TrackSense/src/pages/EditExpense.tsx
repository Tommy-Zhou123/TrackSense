...
// expenses row component
function expenseRow({ editableExpenses, setEditableExpenses, expense, checkedExps, setCheckedExps, editMode, editId, setEditId }: any) {
    ...
    function getExpenses() {
        axios.get('http://localhost:3000/expenses')
            .then((response) => {
                ...
                setEditableExpenses(expensesWithDates);
            })
            .catch((err) => {
                console.log(err);
                                ...
                                </tr> */}
                                {
                                    editMode || editId != -1 ? (
                                        ...
                                        expenses?.map((expense: Expense) => {
                                            return (<tr key={expense._id.toString()} onDoubleClick={() => { setEditId(expense._id); setEditableExpenses(expenses); }} >
                                                <td><Form.Check checked={checkedExps.includes(expense._id.toString())} id={expense._id.toString()} className='text-center' onChange={handleCheck} /></td>
